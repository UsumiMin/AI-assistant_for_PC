...
class SmartModel:
    def __init__(self, apps_list,  model_name="Qwen/Qwen2.5-1.5B-Instruct-GGUF", model_filename="qwen2.5-1.5b-instruct-q4_k_m.gguf"):
        ...
        - "target": объект действия (название приложения из доступных приложений, только если действия "run"). Если по контексту поиск в браузере, то пишем цель поиска. Если действие "new", то название файла из запроса. Если такового нет, то просто "файл".
        ...
        Запрос: "Создай файл"
        Ответ: {{"action": "new", "target": "файл", "answer": "Хорошо, создаю файл.", "emotion": "processing"}}

        Запрос: "Создай список покупок"
        Ответ: {{"action": "new", "target": "список покупок", "answer": "Конечно, создаю список покупок.", "emotion": "processing"}}
